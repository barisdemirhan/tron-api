...
class TRC20Contract
{
     ...
     * @param string|null $setFee
     * @return array
     * @throws TRC20Exception
     * @throws TronException
     */
    public function transfer(string $to, string $amount, string $from = null, string $setFee = null): array
    {
        if ($from == null) {
        ...
        $fee = $this->feeLimit;
        if ($setFee != null && $setFee > 0) {
            $fee = $setFee;
        }

        $feeLimitInSun = bcmul((string)$fee, (string)self::TRX_TO_SUN);

        if (!is_numeric($fee) or $fee <= 0) {
